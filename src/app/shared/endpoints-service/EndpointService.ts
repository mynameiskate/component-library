...
export class EndpointService {
  ...
  searchCountryOptions = (searchInput: string) => {
    return _.filter(this.getCountryOptions(),
      (item: IDropdownItem) => this.isSubstrFound(item.value, searchInput));
  }


  getCountryAutocompleteOptions (searchInput: string) {
    return Promise.resolve(this.searchCountryOptions(searchInput))
      .then((options) => (
  ...
  getCountryOptions() {
    const countries: Array<IDropdownItem> = [
